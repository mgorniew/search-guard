...
public abstract class AbstractCryptoManager implements CryptoManager {
    ...
    protected static final List<String> SECURE_TLS_PROTOCOLS;
    
    static {
        Builder<String> builder = ImmutableList.builder();
        builder.add("TLSv1.2", "TLSv1.1");
        if(CryptoManagerFactory.JDK_SUPPORTS_TLSV13) {
            builder.add("TLSv1.3");
        }
        SECURE_TLS_PROTOCOLS = builder.build();
    }
    ...
    public final String generatePBKDF2PasswordHash(char[] plainTextPassword, String provider) throws NoSuchAlgorithmException, NoSuchProviderException, InvalidKeySpecException {
        ...
        return Base64.encodeBase64String(salt)+"#"+Base64.encodeBase64String(key.getEncoded());
    }
    ...
    protected final boolean checkPBKDF2PasswordHash(String hash, char[] plainTextPassword, String provider) throws NoSuchAlgorithmException, NoSuchProviderException, InvalidKeySpecException {
        try {
            ...
            final SecretKey hashedPlaintext = generatePBKDF2Hash(plainTextPassword,Base64.decodeBase64(splittedHash[0]), 10000, provider);
            return MessageDigest.isEqual(Base64.decodeBase64(splittedHash[1]), hashedPlaintext.getEncoded());
        } finally {
